...
import { Box, Card, CardActions, CardContent, Typography, Avatar, Chip, IconButton, Button } from '@mui/material';
import {
            ...
            <Box className='animate__animated animate__zoomIn'>
                <Card variant='outlined'>
                    ...
                    <CardContent sx={{ padding: '16px 16px 8px 16px' }}>
                        ...
                        >
                            <Avatar sx={{ border: 'solid black 1px' }} aria-label='recipe' src={image} />
                            <span className='cryptoName'>{name}</span>
                            <Chip
                                sx={{ marginLeft: 'auto' }}
                                label={numberWithCommas(price)}
                                color={marketHistory.priceChangePercentage24h > 0 ? 'success' : 'error'}
                                icon={<AttachMoneyIcon fontSize='small' />}
                            />
                        </Typography>
                        <Typography component='div' sx={{ marginTop: '5px' }} fontSize='small'>
                            Price History
                            <br />
                    ...
                    <CardActions sx={{ display: 'flex' }}>
                        ...
                        {user ? (
                            <PurchaseModal crypto={crypto} handleUpdateSingleCrypto={handleUpdateSingleCrypto} />
                        ) : (
