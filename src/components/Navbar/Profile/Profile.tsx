...
import { Box, Drawer, Card, CardContent, Avatar, Chip, Typography } from '@mui/material';
import {
    formatPrice,
...
export const Profile = () => {
    ...
    let accountTotalValue = user ? calculateTotalValue(user.portfolio, cryptos, user.balance) : 0;

    let totalInvestmentReturn = calculateTotalInvestmentReturn(accountTotalValue, user ? user.deposits : []);
        ...
        <ProfileDrawerWrapper>
            ...
            >
                <Card variant='outlined'>
                    ...
                    >
                        <UpdateProfileImageForm currentProfilePicture={user ? user.profilePicture : ''} />
                    ...
                    <CardContent className='portfolioSummary' sx={{ padding: '0 16px' }}>
                        <div className='portfolioDetails'>
                            <Avatar sx={{ border: 'solid black 1px', backgroundColor: 'black' }} aria-label='balance'>
                            ...
                            <Chip
                                ...
                                icon={totalInvestmentReturn >= 0 ? <ArrowCircleUpIcon /> : <ArrowCircleDownIcon />}
                            />
                        </div>
                        ...
                        <div className='portfolioBreakdown'>
                            <p>
                                <ArrowRightIcon />
                                Crypto: ${user && numberWithCommas(calculatePortfolioValue(user?.portfolio, cryptos))}
                            </p>
                            <p>
                                <ArrowRightIcon />
                    ...
                    <div className='walletIcon' style={{ marginBottom: '10px' }}>
                        <Chip label='My Wallet' icon={<AccountBalanceWalletIcon fontSize='small' />} />
                    </div>
                    <CryptoWallet />
                </Card>
            </Box>
